...
public class Ex01Board01 {
    ...
    private static void showMenu() {
        ...
        while (true) {
            ...
            if (userChoice == 1) {
                // 게시판을 작성하고 배열에 담아서 리턴해주는 메소드를 실행해서
                // 그 결과값으로 boardArray를 덮어 씌워준다
    ...
    private static Board[] selectAll(Scanner scanner, Board[] boardArray) {
        // 만약 boardArray 안에 아무런 글이 없으면 경고 메세지만 출력하고 끝낸다.
    ...
    private static Board[] selectOne(Scanner scanner, Board[] boardArray, int index) {
        ...
        //Board 변수를 만들어서 보드 배열의 index번과 연결시켜준다.
    ...
    // 해당 인덱스를 삭제하고 다시 호출된 곳으로 배열을 보내주는
    // deleteBoard()
