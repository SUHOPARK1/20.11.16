...
public class Ex02String {
    public static void main(String[] args) {
        ...
        // 1. 기본 생성자 호출
        //    아무런 문자도 안 담겨있는 String 객체가 준비됨
        String str = new String();
        System.out.println("str의 현재값: "+str);
        
        // 2. 파라미터가 있는 생성자 호출
        //    파라미터로 들어간 값이 담긴 String 객체가 준됨
        ...
        // 10. 해당 문자열에서 특정 정규표현식에 따라서
        //     패턴이 맞는지를 체크할 때에는 matches()을 실행하면 된다.
        //     정규표현식이란? = 숙제
        String pattern = "\\d*"; // \\d*은 모든 문자가 숫자로 이루어져있는지를 확인함
        System.out.println("str.matches(pattern): "+str.matches(pattern));
        pattern = "\\w*"; // \\w*은 알파벳 모둔 문자가 알파벳으로 이루어져있는지를 확인함.
        System.out.println("str.matches(pattern): "+str.matches(pattern));
        ...
        // 14. 문자열의 모든 글자를 대문자로 변환할 때애눈 toUpperCase()를 사용한다.
        //      ***주의*** 쪼갠다고 해서 원본이 바뀌지는 않음
