...
public class Ex01GuessingGame {
    ...
    // 현재점수와 최고점수를 비교해서 더 작은 숫자를 return 해주는
    // setBestScore 메소드
    private static int setBestScore(int score, int bestScore) {
        if (bestScore == 0) {
            System.out.println("맨 처음 플레이 하셔서 최고기록을 달성 하셨습니다!");
            return score;
        }
        if (score < bestScore) {
    ...
    private static int play(Scanner scanner, int bestScore) {
        ...
        int userNumber = ScannerUtil.nextInt(scanner, "숫자: ", 1, 100);
