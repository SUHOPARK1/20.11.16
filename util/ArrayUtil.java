...
public class ArrayUtil {
    ...
    public static int[] removeByIndex(int[] array, int index) {
        ...
        // C. 임시 배열에서 i번재 인덱스를 i-1번 인덱스에 넣는 식으로 복사해 온다.
        // D. 줄어든 배열을 리턴한다.
    ...
    // Student 객체에 대한
    // 1. contains
    public static boolean cotains(Student[] array, Student s) {
        for (int i = 0; i < array.length; i++) {
            if (s.equals(array[i])) {
                return true;
            }
    ...
    // 단, i 번째와 element와 같은지 비교할때는
    // id가 같고 name이 같으면 같은 걸로 판정
    // 즉 array[i].id == elemnet.id && array[i].name.equals(element.name)
